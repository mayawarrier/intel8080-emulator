...
#include "i8080_predef.h"
 ...
 /* For internal use */
static const i8080_word_t WORD_LO_F = ((i8080_word_t)1 << HALF_WORD_SIZE) - (i8080_word_t)1;
static const i8080_word_t WORD_HI_F = (((i8080_word_t)1 << HALF_WORD_SIZE) - (i8080_word_t)1) << HALF_WORD_SIZE;
static const i8080_dbl_word_t BUF_HI_WORD_MAX = (i8080_dbl_word_t)WORD_T_MAX << (HALF_WORD_SIZE * 2);
/* Conditional RETs and CALLs (subroutine ops) take 6 cycles longer if the condition is true. */
static const i8080_uintmax_t SUBROUTINE_CYCLES_OFFSET = 6;
...
/* Get value of a particular bit position in an i8080_word_t. */
static inline int get_word_bit(i8080_word_t word, int bit) { 
    return ((word >> bit) & 1); 
}
/* Get value of a particular bit position in an i8080_dbl_word_t. */
static inline int get_buf_bit(i8080_dbl_word_t buf, int bit) { 
    return ((buf >> bit) & 1); 
}
/* Set value of a particular bit position in an i8080_word_t. */
static inline void set_word_bit(i8080_word_t * word, int bit, int val) {
    if (val) {
        *word |= ((i8080_word_t)1 << bit);
    } else {
        *word &= ~((i8080_word_t)1 << bit);
    }
}
...
static inline i8080_word_t word_lo_bits(i8080_word_t word) {
    return (word & WORD_LO_F);
}
/* Picks the upper half of the word. */
static inline i8080_word_t word_hi_bits(i8080_word_t word) {
    return (word & WORD_HI_F);
}
/* Picks the word bits from the hi part of buf_t. */
static inline i8080_word_t buf_hi_word(i8080_dbl_word_t buf) {
    return (i8080_word_t)((buf & BUF_HI_WORD_MAX) >> (2 * HALF_WORD_SIZE));
}
...
static inline i8080_dbl_word_t twos_comp_word(i8080_word_t word) {
    return ((i8080_word_t)(~word) + (i8080_dbl_word_t)1);
}
...
/* Gets the bit just after the width of an i8080_word_t. */
static inline int get_carry_bit(i8080_dbl_word_t buf) {
...
static inline i8080_dbl_word_t concatenate(i8080_word_t word1, i8080_word_t word2) {
    return ((i8080_dbl_word_t)word1 << HALF_ADDR_SIZE | word2);
}
...
static inline void i8080_set_psw(i8080 * const cpu, i8080_dbl_word_t dbl_word) {
    cpu->a = (i8080_word_t)buf_hi_word(dbl_word);
...
static void i8080_rlc(i8080 * const cpu) {
    int msb = get_word_bit(cpu->a, HALF_WORD_SIZE * 2 - 1);
    /* shift left */
    cpu->a <<= 1;
    /* set lsb to msb */
...
static void i8080_ral(i8080 * const cpu) {
    ...
    int msb = get_word_bit(cpu->a, 2 * HALF_WORD_SIZE - 1);
    cpu->a <<= 1;
    cpu->cy = msb;
...
static void i8080_push(i8080 * const cpu, i8080_dbl_word_t dbl_word) {
    i8080_word_t left_word = (i8080_word_t)buf_hi_word(dbl_word);
